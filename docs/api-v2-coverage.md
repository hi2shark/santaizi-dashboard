...
| 流量策略 | `createServer`/`updateServer` 可选 `traffic_policies` 原子提交；`listTrafficPolicies`, `createTrafficPolicy`, `updateTrafficPolicy`, `deleteTrafficPolicy`, `getTrafficPolicyUsage` | 服务器编辑器内多策略卡片，随服务器一次保存 | 累计/周期策略与用量进度；缺名称不发写请求；后端 400 时服务器未创建 |
