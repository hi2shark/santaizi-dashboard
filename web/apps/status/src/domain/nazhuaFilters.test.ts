...
function row(id: number, name: string, tag: string, displayIndex: number, online: boolean, country: string, platform: string): ServerRecord {
  return {
    id, name, tag, display_index: displayIndex, online,
    hide_for_guest: false, enable_ddns: false,
    host: { CountryCode: country, Platform: platform },
  }
}
