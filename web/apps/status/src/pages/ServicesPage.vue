  ...
  <div class="status-container">
    <section class="status-panel service-panel">
      ...
      <div v-else class="empty-status">
        <AppEmpty :icon="failed ? 'ri-error-warning-line' : 'ri-heart-pulse-line'" :description="t(failed ? 'loadFailed' : loading ? 'loading' : 'noData')" />
        <el-button v-if="failed" @click="load"><i class="ri-refresh-line"></i>{{ t('refresh') }}</el-button>
      </div>
    </section>
  </div>
</template>
